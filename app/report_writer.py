...
def _to_markdown(report: ReviewReport) -> str:
    ...
    if not report.top_issues:
        ...
        for issue in report.top_issues[:8]:
            cell_str = f" [Cell {issue.cell_number}]" if issue.cell_number else ""
            lines.extend(
                [
                    f"- [{issue.severity}] {issue.file_path}{cell_str} {issue.line_hint or ''}".strip(),
    ...
    if not grouped:
        ...
        for file_path, issues in grouped.items():
            ...
            for issue in issues:
                cell_str = f" [Cell {issue.cell_number}]" if issue.cell_number else ""
                lines.append(
                    f"- [{issue.severity}/{issue.category}] {issue.title} at {issue.line_hint or 'N/A'}{cell_str} "
    ...
    for review in report.provider_results:
        if not review.summary and not review.concerns and not review.error:
            continue
        lines.append(f"### {review.role.title()} Reviewer ({review.provider})")
        if review.error:
            lines.append(f"Status: Failed — {review.error}")
        else:
            if review.summary:
                lines.append(review.summary)
            for concern in review.concerns[:3]:
